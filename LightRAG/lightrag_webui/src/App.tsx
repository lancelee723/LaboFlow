    ...
    const checkVersion = async () => {
      ...
      try {
        ...
        if (!status.auth_configured && status.access_token) {
          // Don't override an existing SSO session (platform_admin etc.) with a guest token
          const currentState = useAuthStore.getState();
          if (token && currentState.isAuthenticated && !currentState.isGuestMode) {
            // Keep existing SSO token, just update version info
            ...
            useAuthStore.getState().login(
              status.access_token, // Use the new guest token
              true, // Guest mode
              status.core_version,
              status.api_version,
              status.webui_title || null,
