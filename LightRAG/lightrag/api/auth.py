...
class AuthHandler:
    ...
    def validate_external_token(self, token: str) -> dict:
        """Validate an externally-issued JWT (e.g. from Clawith SSO).

        Uses the same secret and algorithm but does NOT check against local accounts.
        Returns the raw payload dict or raises HTTPException.
        """
        try:
