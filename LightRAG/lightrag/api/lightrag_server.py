...
def create_app(args):
    ...
    # Load runtime overlay — overrides .env defaults without a full restart.
    ...
    def create_rerank_func():
        """Build a rerank callable from current args.  Returns None when disabled.

        Called both at startup and by FactoryBundle.make_rerank_func for hot-swap.
        """
        if not args.rerank_binding or args.rerank_binding == "null":
            return None
        ...
        selected = rerank_functions.get(args.rerank_binding)
        if not selected:
            raise ValueError(f"Unsupported rerank binding: {args.rerank_binding}")

        # Fill in defaults from provider signature when not explicitly set.
        ...
        async def server_rerank_func(
            ...
            """Server rerank function with configuration from args"""
            kwargs = {
                "query": query,
                "documents": documents,
                "top_n": top_n,
            ...
            return await selected(**kwargs, extra_body=extra_body)

        return server_rerank_func

    # Configure rerank function based on args.rerank_binding
    ...
    # Wire LLM provider config routes (admin-only hot-swap and rebuild)
    app.state.rag = rag
    app.state.args = args
    ...
    app.state.factories = FactoryBundle(
        ...
        make_rerank_func=create_rerank_func,
    )
    app.include_router(llm_config_router)

    # Custom Swagger UI endpoint for offline support.
    # LaboFlow note: in the LaboFlow reverse-proxy setup, LightRAG API calls
    # are namespaced under /kb-api/ (see nginx.conf). The Swagger UI HTML is
    # loaded via an iframe at /kb-api/docs and must reference its assets
    # inside the same namespace, otherwise the browser requests absolute
    # paths like /static/swagger-ui/... that nginx forwards to the Clawith
    # frontend and result in 404. These paths are benign when LightRAG is
    # run standalone: nginx proxying is transparent and /kb-api/... maps to
    # the same routes after prefix stripping.
    @app.get("/docs", include_in_schema=False)
    async def custom_swagger_ui_html():
        """Custom Swagger UI HTML with local static files"""
        return get_swagger_ui_html(
            openapi_url="/kb-api/openapi.json",
            title=app.title + " - Swagger UI",
            oauth2_redirect_url="/kb-api/docs/oauth2-redirect",
            swagger_js_url="/kb-api/static/swagger-ui/swagger-ui-bundle.js",
            swagger_css_url="/kb-api/static/swagger-ui/swagger-ui.css",
            swagger_favicon_url="/kb-api/static/swagger-ui/favicon-32x32.png",
    ...
    async def sso_login(request: Request):
        """SSO login: accept a Clawith JWT, validate it, and issue a LightRAG token.

        Expects JSON body: {"token": "<clawith_jwt>"}
        The Clawith JWT is validated using the shared TOKEN_SECRET.
        Returns same format as /login so the WebUI can handle it uniformly.
        """
        ...
        # Clawith token has sub=user_id (UUID), role=platform_role
        user_id = payload.get("sub", "sso_user")
        # Use a readable username for LightRAG display
    ...
    if webui_assets_exist:
        ...
        logger.info("WebUI assets mounted at /kb")
    else:
        logger.info("WebUI assets not available, /kb route not mounted")

        # Add redirect for /kb when assets are not available
        @app.get("/kb")
        @app.get("/kb/")
        async def webui_redirect_to_docs():
            """Redirect /kb to /docs when WebUI is not available"""
            return RedirectResponse(url="/docs")
