...
AIPPT_SETTING_KEY = "aippt_llm_config"

AIPPT_PROVIDERS = {
    "presets": [
        {"key": "deepseek", "label": "DeepSeek", "base_url": "https://api.deepseek.com/v1",
         "models": [{"label": "DeepSeek Chat", "value": "deepseek-chat"}, {"label": "DeepSeek Reasoner", "value": "deepseek-reasoner"}]},
        {"key": "kimi", "label": "Kimi (Moonshot)", "base_url": "https://api.moonshot.cn/v1",
         "models": [{"label": "Kimi K2 0905 Preview", "value": "kimi-k2-0905-preview"}, {"label": "Kimi K2 0711", "value": "kimi-k2-0711"}]},
        {"key": "glm", "label": "智谱AI (GLM)", "base_url": "https://open.bigmodel.cn/api/paas/v4",
         "models": [{"label": "GLM-4.7", "value": "glm-4.7"}, {"label": "GLM-4 Plus", "value": "glm-4-plus"}, {"label": "GLM-4 Air", "value": "glm-4-air"}, {"label": "GLM-4 Flash", "value": "glm-4-flash"}]},
        {"key": "qwen", "label": "阿里千问 (Qwen)", "base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1",
         "models": [{"label": "Qwen 3 Max", "value": "qwen3-max"}, {"label": "Qwen Max", "value": "qwen-max"}, {"label": "Qwen Plus", "value": "qwen-plus"}, {"label": "Qwen Turbo", "value": "qwen-turbo"}]},
        {"key": "doubao", "label": "豆包 (Doubao)", "base_url": "https://ark.cn-beijing.volces.com/api/v3",
         "models": [{"label": "Doubao Seed 1.6", "value": "doubao-seed-1.6"}, {"label": "Doubao Seed 1.6 Thinking", "value": "doubao-seed-1.6-thinking"}, {"label": "Doubao 1.5 Pro", "value": "doubao-1.5-pro"}]},
        {"key": "minimax", "label": "MiniMax", "base_url": "https://api.minimaxi.com/v1",
         "models": [{"label": "MiniMax M2.1", "value": "minimax-m2.1"}, {"label": "MiniMax M2.1 Lightning", "value": "minimax-m2.1-lightning"}, {"label": "MiniMax M2", "value": "minimax-m2"}]},
        {"key": "openai", "label": "OpenAI", "base_url": "https://api.openai.com/v1",
         "models": [{"label": "GPT-5.2", "value": "gpt-5.2"}, {"label": "GPT-4o", "value": "gpt-4o"}, {"label": "O1", "value": "o1"}, {"label": "O1 Mini", "value": "o1-mini"}]},
        {"key": "claude", "label": "Claude (Anthropic)", "base_url": "https://api.anthropic.com/v1",
         "models": [{"label": "Claude Sonnet 4.5", "value": "claude-sonnet-4-5"}, {"label": "Claude Opus 4.5", "value": "claude-opus-4-5"}, {"label": "Claude 3.5 Sonnet", "value": "claude-3-5-sonnet-20241022"}]},
        {"key": "gemini", "label": "Gemini (Google)", "base_url": "https://generativelanguage.googleapis.com/v1beta/openai",
         "models": [{"label": "Gemini 3 Pro", "value": "gemini-3-pro-preview"}, {"label": "Gemini 2.5 Pro", "value": "gemini-2.5-pro"}, {"label": "Gemini 2.5 Flash", "value": "gemini-2.5-flash"}, {"label": "Gemini 2.5 Flash-Lite", "value": "gemini-2.5-flash-lite"}]},
        {"key": "grok", "label": "Grok (xAI)", "base_url": "https://api.x.ai/v1",
         "models": [{"label": "Grok 4", "value": "grok-4"}, {"label": "Grok 3", "value": "grok-3"}, {"label": "Grok 3 Mini", "value": "grok-3-mini"}]},
        {"key": "custom", "label": "Custom (OpenAI-compatible)", "base_url": "",
         "models": []},
    ],
}
...
def _mask_aippt_key(key: str) -> str:
    if not key or len(key) < 8:
        return "••••••••" if key else ""
    return key[:4] + "••••" + key[-4:]


def _aippt_sentinel_check(submitted: str, existing: str) -> str:
    if submitted and "••••" in submitted:
        return existing
    return submitted


class AipptLlmConfigIn(BaseModel):
...
@router.get("/aippt-llm-config")
async def get_aippt_llm_config(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
        select(SystemSetting).where(SystemSetting.key == AIPPT_SETTING_KEY)
    )
    setting = result.scalar_one_or_none()
    raw = setting.value if setting else {}
    config = {**AIPPT_DEFAULTS, **raw}

    masked_api_keys: dict[str, str] = {}
    for k, v in config.get("apiKeys", {}).items():
        masked_api_keys[k] = _mask_aippt_key(v)
    ...
    return {
        ...
        "apiKeysMasked": masked_api_keys,
        "source": "db" if setting else "defaults",
        "providers": AIPPT_PROVIDERS,
    }


@router.put("/aippt-llm-config")
async def update_aippt_llm_config(
    data: AipptLlmConfigIn,
    current_user: User = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
    ...
    setting = result.scalar_one_or_none()
    existing = setting.value if setting else {}

    existing_api_keys: dict[str, str] = existing.get("apiKeys", {})
    merged_api_keys: dict[str, str] = {}
    for k, v in data.apiKeys.items():
        merged_api_keys[k] = _aippt_sentinel_check(v, existing_api_keys.get(k, ""))

    for k, v in existing_api_keys.items():
        if k not in merged_api_keys:
            merged_api_keys[k] = v
    ...
    new_value = {
        ...
        "apiKeys": {k: v for k, v in merged_api_keys.items() if v},
    }

    if setting:
        setting.value = new_value
    else:
        setting = SystemSetting(key=AIPPT_SETTING_KEY, value=new_value)
        db.add(setting)
    await db.commit()

    masked_api_keys = {k: _mask_aippt_key(v) for k, v in new_value.get("apiKeys", {}).items()}
    return {
        "provider": new_value["provider"],
        "model": new_value["model"],
        "temperature": new_value["temperature"],
        "baseUrl": new_value["baseUrl"],
        "apiKeysMasked": masked_api_keys,
        "source": "db",
    }
...
async def get_aippt_llm_runtime(
    ...
    setting = result.scalar_one_or_none()
    if not setting or not setting.value:
        return {**AIPPT_DEFAULTS, "apiKeys": {}, "providers": AIPPT_PROVIDERS}

    config = {**AIPPT_DEFAULTS, **setting.value}
    return {
        "provider": config.get("provider", "deepseek"),
        "model": config.get("model", "deepseek-chat"),
        "temperature": config.get("temperature", 0.7),
