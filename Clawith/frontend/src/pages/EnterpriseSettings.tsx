...
interface AipptConfigOut {
    provider: string; model: string; temperature: number; baseUrl: string;
    apiKeysMasked: Record<string, string>; source: 'db' | 'defaults';
    providers: { presets: AipptProviderPreset[] };
}
...
function AipptTab() {
    ...
    const [allMaskedKeys, setAllMaskedKeys] = useState<Record<string, string>>({});
    const [presets, setPresets] = useState<AipptProviderPreset[]>([]);
    const [configSource, setConfigSource] = useState<'db' | 'defaults'>('defaults');
    ...
    const loadConfig = async () => {
        ...
        setError(null);
        try {
            const data = await fetchJson<AipptConfigOut>('/enterprise/aippt-llm-config');
            setProvider(data.provider);
            setModel(data.model);
            setTemperature(data.temperature);
            setBaseUrl(data.baseUrl);
            setAllMaskedKeys(data.apiKeysMasked);
            setAllApiKeys({ ...data.apiKeysMasked });
            setApiKey(data.apiKeysMasked[data.provider] || '');
            setApiKeyMasked(data.apiKeysMasked[data.provider] || '');
            setPresets(data.providers.presets);
            setConfigSource(data.source);
        } catch (err: any) {
            setError(t('enterprise.aipptConfig.loadError'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleProviderChange = (newProvider: string) => {
        setAllApiKeys({ ...allApiKeys, [provider]: apiKey });
        setProvider(newProvider);
        const preset = presets.find(p => p.key === newProvider);
        if (preset) {
            setBaseUrl(preset.base_url);
            if (preset.models.length > 0 && !preset.models.some(m => m.value === model)) {
                setModel(preset.models[0].value);
            }
        }
        setApiKey(allApiKeys[newProvider] || '');
        setApiKeyMasked(allMaskedKeys[newProvider] || '');
    };

    const handleApiKeyChange = (val: string) => {
        setApiKey(val);
        setAllApiKeys({ ...allApiKeys, [provider]: val });
    };
    ...
    const handleSave = async () => {
        ...
        setError(null);
        setSuccessMsg(null);
        const finalApiKeys = { ...allApiKeys, [provider]: apiKey };
        try {
            await fetchJson('/enterprise/aippt-llm-config', {
                method: 'PUT',
                body: JSON.stringify({ provider, model, temperature, baseUrl, apiKeys: finalApiKeys }),
            });
            setSuccessMsg(t('enterprise.aipptConfig.saveSuccess'));
            setTimeout(() => setSuccessMsg(null), 3000);
            await loadConfig();
        } catch (err: any) {
            setError(err?.message || t('enterprise.aipptConfig.saveError'));
        } finally {
            setSaving(false);
        }
    };
    ...
    const badgeDb: React.CSSProperties = { display: 'inline-block', fontSize: '10px', padding: '1px 6px', borderRadius: '4px', background: '#dbeafe', color: '#1d4ed8', marginLeft: '8px', verticalAlign: 'middle' };
    const badgeDefaults: React.CSSProperties = { ...badgeDb, background: '#f3f4f6', color: '#6b7280' };

    if (loading) {
        return <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary, #6b7280)' }}>{t('enterprise.aipptConfig.loading')}</div>;
    }
        ...
        <div>
            ...
            <div style={cardStyle}>
                <h3 style={{ marginBottom: '16px', fontSize: '15px', fontWeight: 600 }}>
                    {t('enterprise.aipptConfig.defaultProviderTitle')}
                    <span style={configSource === 'db' ? badgeDb : badgeDefaults}>{configSource === 'db' ? t('enterprise.aipptConfig.sourceDb') : t('enterprise.aipptConfig.sourceDefaults')}</span>
                </h3>
                <div style={rowStyle}>
                    <label style={labelStyle}>{t('enterprise.aipptConfig.provider')}</label>
                    <select style={selectStyle} value={provider} onChange={e => handleProviderChange(e.target.value)}>
                        {presets.map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
                    </select>
                </div>
                <div style={rowStyle}>
                    <label style={labelStyle}>{t('enterprise.aipptConfig.model')}</label>
                    {provider === 'custom' ? (
                        <input style={inputStyle} value={model} onChange={e => setModel(e.target.value)} placeholder={t('enterprise.aipptConfig.modelPlaceholder')} />
                    ) : (
                        <select style={selectStyle} value={model} onChange={e => setModel(e.target.value)}>
                            {currentPreset?.models.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                            {!currentPreset?.models.some(m => m.value === model) && <option value={model}>{model}</option>}
                        </select>
                    )}
                </div>
                <div style={rowStyle}>
                    <label style={labelStyle}>{t('enterprise.aipptConfig.baseUrl')}</label>
                    <input style={inputStyle} value={baseUrl} onChange={e => setBaseUrl(e.target.value)} placeholder="https://..." />
                </div>
                <div style={rowStyle}>
                    <label style={labelStyle}>{t('enterprise.aipptConfig.apiKey')}</label>
                    <input
                        style={inputStyle}
                        type="password"
                        value={apiKey}
                        onChange={e => handleApiKeyChange(e.target.value)}
                        placeholder={apiKeyMasked || t('enterprise.aipptConfig.apiKeyPlaceholder')}
                    />
                </div>
                <div style={rowStyle}>
                    <label style={labelStyle}>{t('enterprise.aipptConfig.temperature')}</label>
                    <input style={inputStyle} type="number" step="0.1" min="0" max="2" value={temperature} onChange={e => setTemperature(e.target.value ? parseFloat(e.target.value) : 0.7)} />
                </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '16px' }}>
                <button className="btn btn-primary" disabled={loading || saving} onClick={handleSave}>
                    {saving ? t('enterprise.aipptConfig.saving') : t('enterprise.aipptConfig.save')}
                </button>
            </div>
        </div>
    );
...
export default function EnterpriseSettings() {
    ...
    type TabKey = 'llm' | 'org' | 'info' | 'approvals' | 'audit' | 'tools' | 'skills' | 'quotas' | 'users' | 'invites' | 'aippt' | 'okr';
    const VALID_TABS: TabKey[] = ['info', 'llm', 'tools', 'skills', 'okr', 'aippt', 'invites', 'quotas', 'users', 'org', 'approvals', 'audit'];
