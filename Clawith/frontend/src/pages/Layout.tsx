...
export default function Layout() {
    ...
    const openWeKnoraSSO = useCallback(async () => {
        if (kbLoading) return;
        setKbLoading(true);
        try {
            const token = localStorage.getItem('token');
            const res = await fetch('/api/enterprise/weknora/sso-token', {
                headers: token ? { Authorization: `Bearer ${token}` } : {},
            });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const { token: ssoToken, weknora_url } = await res.json();
            const safeBase = resolveWeKnoraBrowserUrl(weknora_url || '');
            window.open(`${safeBase}/sso?token=${encodeURIComponent(ssoToken)}`, '_blank', 'noopener,noreferrer');
        } catch (err) {
            console.error('[WeKnora SSO] failed to open knowledge base:', err);
