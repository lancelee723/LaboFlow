...
def test_extract_duplicate_pg_type_name_from_postgres_error():
    error = Exception(
        ...
        "DETAIL:  Key (typname, typnamespace)=(sponsor_profiles, 2200) already exists."
    )

    assert _extract_duplicate_pg_type_name(error) == "sponsor_profiles"
...
def test_drop_postgres_orphan_composite_type_drops_when_orphan_exists():
    ...
    assert '"public"."sponsor_profiles"' in connection.executed[1][0]
