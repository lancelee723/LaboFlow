...
class DatabaseMigration:
    ...
    def _register_migrations(self):
        ...
        self.migrations.append({
            ...
            "description": "Add daily check-ins, sponsor profiles, and registration source fields",
        ...
        # Migration 013: Drop invite code system
        self.migrations.append({
            "version": "013",
            "name": "drop_invite_code_system",
            "description": "Remove invite code tables, columns, indexes, and settings",
            "up": self._migration_013_up,
            "down": self._migration_013_down,
        })

        # Migration 014: Add clawith_id for SSO integration
        self.migrations.append({
            "version": "014",
            "name": "add_clawith_id_to_users",
            "description": "Add clawith_id column and unique index to users table for Clawith SSO",
    ...
    async def _migration_011_down(self, session: AsyncSession):
        ...
        try:
            ...
            if dialect.startswith("postgres"):
                ...
                logger.warning("SQLite DROP COLUMN not supported; keeping users.registration_channel")
    ...
    async def _migration_013_up(self, session: AsyncSession):
        """Migration 013: drop invite code system tables, columns, indexes, and settings."""
        logger.info("Applying migration 013: Dropping invite code system")
        try:
            ...
            if dialect.startswith("postgres"):
                ...
                logger.warning("SQLite DROP COLUMN not supported; keeping users.invite_code_id as orphan column")

            if await self._table_exists(session, "user_configs"):
                await session.execute(
                    text(
                        "DELETE FROM user_configs "
                        "WHERE config_key = 'invite_code_required_for_registration' "
                        "AND category = 'community_ops'"
                    )
                )
    ...
    async def _migration_013_down(self, session: AsyncSession):
        """Migration 013 rollback: no-op (invite code data is irreversibly lost)."""
        logger.warning("Migration 013 rollback is a no-op; invite code data cannot be restored")
        pass

    async def _migration_014_up(self, session: AsyncSession):
        """Migration 014: add clawith_id column and unique index to users for SSO."""
        logger.info("Applying migration 014: Adding clawith_id to users")
        try:
            if not await self._column_exists(session, "users", "clawith_id"):
                dialect = self._dialect_name(session)
                if dialect.startswith("postgres"):
                    await session.execute(
                        text("ALTER TABLE users ADD COLUMN clawith_id VARCHAR(100)")
                    )
                else:
                    await session.execute(
                        text("ALTER TABLE users ADD COLUMN clawith_id VARCHAR(100)")
                    )
                logger.info("Added clawith_id column to users")
            else:
                logger.info("clawith_id column already exists in users")

            await session.execute(
                text(
                    "CREATE UNIQUE INDEX IF NOT EXISTS idx_users_clawith_id "
                    "ON users(clawith_id) WHERE clawith_id IS NOT NULL"
                )
            )
    ...
    async def _migration_014_down(self, session: AsyncSession):
        """Migration 014 rollback: remove clawith_id column and index."""
        logger.info("Rolling back migration 014: Removing clawith_id from users")
        try:
            await session.execute(text("DROP INDEX IF EXISTS idx_users_clawith_id"))

            dialect = self._dialect_name(session)
            if dialect.startswith("postgres"):
                await session.execute(text("ALTER TABLE users DROP COLUMN IF EXISTS clawith_id"))
            else:
                logger.warning("SQLite DROP COLUMN not supported; keeping users.clawith_id as orphan column")
