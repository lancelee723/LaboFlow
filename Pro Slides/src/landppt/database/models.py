...
class User(Base):
    ...
    clawith_id: Mapped[Optional[str]] = mapped_column(String(100), unique=True, index=True, nullable=True)
    oauth_provider: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)  # 'github', 'linuxdo', 'clawith' or null for local
...
def _user_metrics_after_credit_transaction_insert(mapper, connection, target) -> None:
    activity_types = {"consume", "redemption", "daily_checkin"}
    activity_ts = None
