...
async function loadTemplates(page = 1) {
    ...
    try {
        ...
        const data = await apiClient.get((window.PPT_PREFIX || "") + '/api/global-master-templates/', params);
...
async function loadTemplatePreview(templateId, iframe) {
    ...
    try {
        ...
        const template = await apiClient.get((window.PPT_PREFIX || "") + `/api/global-master-templates/${templateId}`);
...
async function updateTagFilter() {
    ...
    try {
        do {
            const data = await apiClient.get((window.PPT_PREFIX || "") + '/api/global-master-templates/', {
                active_only: 'true',
...
async function handleTemplateSubmit(event) {
    ...
    try {
        if (state.editingTemplateId) {
            await apiClient.put((window.PPT_PREFIX || "") + `/api/global-master-templates/${state.editingTemplateId}`, payload);
            emit('templates:updated', { action: 'update', id: state.editingTemplateId });
        } else {
            await apiClient.post((window.PPT_PREFIX || "") + '/api/global-master-templates/', payload);
...
async function handleAIGeneration(event) {
    ...
    try {
        ...
        const result = await apiClient.post((window.PPT_PREFIX || "") + '/api/global-master-templates/generate', payload);
...
async function saveGeneratedTemplate() {
    ...
    try {
        await apiClient.post((window.PPT_PREFIX || "") + '/api/global-master-templates/save-generated', state.generatedTemplate);
        alert('模板已保存');
        closeAIGenerationModal();
        loadTemplates(1);
    } catch (error) {
...
async function adjustTemplate() {
    ...
    try {
        ...
        const result = await apiClient.post((window.PPT_PREFIX || "") + '/api/global-master-templates/adjust-template', {
            adjustment,
...
async function previewTemplateById(templateId) {
    try {
        const data = await apiClient.get((window.PPT_PREFIX || "") + `/api/global-master-templates/${templateId}/preview`);
...
async function duplicateTemplate(templateId) {
    ...
    if (!newName) return;
    try {
        await apiClient.post((window.PPT_PREFIX || "") + `/api/global-master-templates/${templateId}/duplicate?new_name=${encodeURIComponent(newName.trim())}`);
...
async function setDefaultTemplate(templateId) {
    ...
    try {
        await apiClient.post((window.PPT_PREFIX || "") + `/api/global-master-templates/${templateId}/set-default`);
        loadTemplates(state.currentPage);
...
async function deleteTemplate(templateId) {
    ...
    try {
        await apiClient.del((window.PPT_PREFIX || "") + `/api/global-master-templates/${templateId}`);
...
async function handleTemplateImport(event) {
    ...
    try {
        ...
        await apiClient.post((window.PPT_PREFIX || "") + '/api/global-master-templates/', templateData);
        event.target.value = '';
        loadTemplates(1);
        alert('模板导入成功');
    } catch (error) {
