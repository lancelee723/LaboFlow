...
async function regenerateSelectedImage() {
    ...
    try {
        ...
        // 发送重新生成请求
        const response = await fetch((window.PPT_PREFIX || "") + '/api/ai/regenerate-image', {
            method: 'POST',
            headers: {
...
async function autoRepairSlideLayout() {
    ...
    try {
        ...
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${targetProjectId}/slides/${currentSlideIndex + 1}/auto-repair-layout`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function autoGenerateSlideImages() {
    ...
    try {
        ...
        // 发送一键配图请求
        const response = await fetch((window.PPT_PREFIX || "") + '/api/ai/auto-generate-slide-images', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
