...
async function showChangeTemplateDialog() {
    ...
    try {
        ...
        const [selectedResp, freeResp] = await Promise.all([
            fetch((window.PPT_PREFIX || "") + `/api/projects/${projectId}/selected-global-template`),
            fetch((window.PPT_PREFIX || "") + `/api/projects/${projectId}/free-template`)
        ]);
...
async function _tplFetchAllLibraryTemplates() {
    ...
    do {
        let url = (window.PPT_PREFIX || "") + `/api/global-master-templates/?page=${page}&page_size=100&active_only=true`;
        if (_tplSearchTerm) {
...
async function _tplApply() {
    ...
    try {
        ...
        const selectResp = await fetch((window.PPT_PREFIX || "") + `/api/projects/${projectId}/select-template`, {
            method: 'POST',
        ...
        const regenResp = await fetch((window.PPT_PREFIX || "") + `/api/projects/${projectId}/slides/batch-regenerate/async`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function _tplGetTemplateDetail(templateId) {
    ...
    const requestTask = (async () => {
        const resp = await fetch((window.PPT_PREFIX || "") + `/api/global-master-templates/${templateId}`);
        const data = await _tplReadJsonResponse(resp);
        if (!resp.ok) {
