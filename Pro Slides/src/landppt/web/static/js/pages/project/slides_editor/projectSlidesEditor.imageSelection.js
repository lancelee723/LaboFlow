...
function getImageUrl(image) {
    ...
    // 如果已经是完整URL，直接返回
    if (image.url && (image.url.startsWith('http') || image.url.startsWith((window.PPT_PREFIX || "") + '/api/'))) {
        return image.url;
    }

    // 如果有image.id，构建API URL
    if (image.id) {
        return (window.PPT_PREFIX || "") + `/api/image/view/${image.id}`;
    }

    // 如果有相对路径，转换为API URL
    if (image.url) {
        return image.url.startsWith('/') ? image.url : (window.PPT_PREFIX || "") + `/api/image/view/${image.url}`;
    }
...
function handleImageError(imgElement, imageId) {
    ...
    if (!originalSrc.includes((window.PPT_PREFIX || "") + '/api/image/view/')) {
        const apiUrl = (window.PPT_PREFIX || "") + `/api/image/view/${imageId}`;
    ...
    if (container) {
        container.innerHTML = `
            <div class="text-center text-muted py-5">
                ...
                <h5>图片库中暂无图片</h5>
                <p>您可以先上传一些图片到图片库</p>
                <a href=(window.PPT_PREFIX || "") + "/image-gallery" class="btn btn-outline-primary" target="_blank">
