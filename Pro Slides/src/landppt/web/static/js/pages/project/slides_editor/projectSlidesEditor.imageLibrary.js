        ...
        async function loadLibraryImages(searchTerm = '', page = 1) {
            try {
                ...
                const response = await fetch((window.PPT_PREFIX || "") + `/api/image/gallery/list?${params}`);
        ...
        function renderLibraryImages() {
            ...
            filteredLibraryImages.forEach(image => {
                ...
                // 构建图片URL
                const imageUrl = (window.PPT_PREFIX || "") + `/api/image/view/${image.image_id}`;
        ...
        async function getImageCompleteInfo(imageId) {
            try {
                const response = await fetch((window.PPT_PREFIX || "") + `/api/image/${imageId}/info`);
                const result = await response.json();
                ...
                // 如果专用接口失败，尝试使用详情接口
                try {
                    const detailResponse = await fetch((window.PPT_PREFIX || "") + `/api/image/detail/${imageId}`);
