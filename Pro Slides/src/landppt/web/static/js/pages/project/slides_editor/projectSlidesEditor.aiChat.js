...
async function saveSlideOutline() {
    ...
    try {
        // 保存大纲到数据库
        const response = await fetch((window.PPT_PREFIX || "") + `/projects/${window.landpptEditorConfig.projectId}/update-outline`, {
            method: 'POST',
            headers: {
...
async function getSelectedGlobalTemplate() {
    try {
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/selected-global-template`);
        if (!response.ok) {
            return null;
        }
        const data = await response.json();
...
async function sendAIMessage(options = {}) {
    ...
    try {
        ...
        // 发送流式AI编辑请求
        const response = await fetch((window.PPT_PREFIX || "") + '/api/ai/slide-edit/stream', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
