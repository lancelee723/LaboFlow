...
async function exportToPDF() {
    ...
    try {
        // 使用新的异步导出端点
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/export/pdf/async`, {
            method: 'POST'
        });
...
async function trackPdfExportTask(taskId, progressToast) {
    return new Promise((resolve, reject) => {
        ...
        const poll = async () => {
            attempts += 1;
            try {
                const response = await fetch((window.PPT_PREFIX || "") + `/api/landppt/tasks/${taskId}`);
                if (!response.ok) {
                ...
                if (data.status === 'completed' && data.result && data.result.success) {
                    ...
                    const downloadUrl = data.download_url || (window.PPT_PREFIX || "") + `/api/landppt/tasks/${taskId}/download`;
...
function exportToPDFClientSide() {
    ...
    // window.open((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/export/pdf?fallback=true`, '_blank');
}
...
async function exportToPPTX() {
    ...
    try {
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/export/pptx`);
        if (!response.ok) {
            throw new Error(`服务返回异常(${response.status})`);
        }
...
async function exportToPPTXAsImages() {
    ...
    try {
        ...
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/export/pptx-images`, {
            method: 'POST',
            headers: {
...
function downloadHTML() {
    ...
    downloadLink.href = (window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/export/html`;
...
async function showShareDialog() {
    try {
        ...
        // 调用API生成分享链接
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/share/generate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
...
async function disableSharing() {
    try {
        const response = await fetch((window.PPT_PREFIX || "") + `/api/projects/${window.landpptEditorConfig.projectId}/share/disable`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
