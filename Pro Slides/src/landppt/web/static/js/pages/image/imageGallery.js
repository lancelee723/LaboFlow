...
async function loadImages(page = 1) {
    try {
        ...
        const data = await apiClient.get((window.PPT_PREFIX || "") + '/api/image/gallery/list', {
            page,
...
async function batchDelete() {
    ...
    try {
        const data = await apiClient.post((window.PPT_PREFIX || "") + '/api/image/gallery/batch-delete', {
...
async function batchDownload() {
    ...
    try {
        const blob = await apiClient.post((window.PPT_PREFIX || "") + '/api/image/gallery/batch-download', {
            image_ids: Array.from(selectedImages)
        }, { responseType: 'blob' });
...
async function clearAllImages() {
    ...
    try {
        const data = await apiClient.post((window.PPT_PREFIX || "") + '/api/image/gallery/clear-all');

        if (data.success) {
            showSuccess(data.message || `成功清空你的图库，删除了 ${data.deleted_count} 张图片`);
...
async function downloadSingleImage(imageId) {
    try {
        const { data: blob, headers } = await apiClient.get((window.PPT_PREFIX || "") + `/api/image/download/${imageId}`, null, { responseType: 'blob', returnResponse: true });
...
async function deleteSingleImage(imageId) {
    ...
    try {
        const data = await apiClient.del((window.PPT_PREFIX || "") + `/api/image/delete/${imageId}`);
...
async function startUpload() {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
            ...
            const data = await apiClient.post((window.PPT_PREFIX || "") + '/api/image/upload', formData);
    ...
    if (uploadedCount > 0) {
        setTimeout(async () => {
            closeUploadModal();
            // New uploads (created_desc) should show immediately
            await loadImages(1);
...
async function showImageDetail(imageId) {
    try {
        const data = await apiClient.get((window.PPT_PREFIX || "") + `/api/image/detail/${imageId}`);
...
async function saveImageInfo() {
    ...
    try {
        const result = await apiClient.put((window.PPT_PREFIX || "") + `/api/image/${currentImageDetail.image_id}/update`, {
            title,
            description,
            tags,
            category
