...
async def github_login(
    ...
    if not enabled:
        ...
        return RedirectResponse(
            url=prefixed("/auth/login?error=GitHub登录未启用"),
            status_code=302
        )
...
async def github_callback(
    ...
    if error:
        ...
        return RedirectResponse(
            url=prefixed(f"/auth/login?error=GitHub授权失败: {error_msg}"),
            status_code=302
        )
    ...
    if not code or not state:
        return RedirectResponse(
            url=prefixed("/auth/login?error=GitHub授权参数缺失"),
            status_code=302
        )
    ...
    if not access_token:
        return RedirectResponse(
            url=prefixed("/auth/login?error=获取GitHub访问令牌失败"),
            status_code=302
        )
    ...
    if not github_user:
        return RedirectResponse(
            url=prefixed("/auth/login?error=获取GitHub用户信息失败"),
            status_code=302
        )
