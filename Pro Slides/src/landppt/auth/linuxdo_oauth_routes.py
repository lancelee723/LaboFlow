...
async def linuxdo_login(
    ...
    if not enabled:
        ...
        return RedirectResponse(
            url=prefixed("/auth/login?error=Linux Do登录未启用"),
            status_code=302
        )
    ...
    # Store state
    await store_oauth_state(state, redirect_url)
...
async def linuxdo_callback(
    ...
    if error:
        ...
        return RedirectResponse(
            url=prefixed(f"/auth/login?error=Linux Do授权失败: {error_msg}"),
            status_code=302
        )
    ...
    if not code or not state:
        return RedirectResponse(
            url=prefixed("/auth/login?error=Linux Do授权参数缺失"),
            status_code=302
        )
    ...
    if not state_data:
        ...
        return RedirectResponse(
            url=prefixed("/auth/login?error=授权请求已过期，请重试"),
            status_code=302
        )
    ...
    if not access_token:
        return RedirectResponse(
            url=prefixed("/auth/login?error=获取Linux Do访问令牌失败"),
            status_code=302
        )
    ...
    if not linuxdo_user:
        return RedirectResponse(
            url=prefixed("/auth/login?error=获取Linux Do用户信息失败"),
            status_code=302
        )
    ...
    if not user:
        return RedirectResponse(
            url=prefixed(f"/auth/login?tab=register&register_error={quote(error_message or '用户创建失败')}"),
            status_code=302
        )
