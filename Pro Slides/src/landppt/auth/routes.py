...
from ..web.template_utils import build_templates
...
router = APIRouter()
templates = build_templates(directory="src/landppt/web/templates")
...
async def _load_system_oauth_flags() -> dict:
    ...
    return {
        ...
        "sso_enabled": bool(app_config.sso_enabled and app_config.sso_jwt_secret),
    }
...
async def login_page(
    ...
    if user:
        return RedirectResponse(url=prefixed("/dashboard"), status_code=302)
...
async def login(
    ...
    try:
        ...
        response = RedirectResponse(url=prefixed("/dashboard"), status_code=302)
...
async def logout(
    ...
    response = RedirectResponse(url=prefixed("/auth/login?success=已成功退出登录"), status_code=302)
    response.delete_cookie("session_id", path=cookie_path())
    ...
    try:
        ...
        if user:
            ...
            return RedirectResponse(
                url=prefixed("/auth/login?success=注册成功，请登录"),
                status_code=302
            )
        else:
...
async def reset_password(
    ...
    try:
        ...
        return RedirectResponse(
            url=prefixed("/auth/login?success=密码重置成功，请登录"),
            status_code=302
        )
    except Exception as e:
