...
app = FastAPI(
    ...
    # NOTE: We intentionally do NOT set root_path here.
    # nginx rewrites /ppt/foo → /foo before forwarding, so the app sees its
    # native paths. The ROOT_PATH env var is consumed by url_utils.py to
    # prefix outgoing redirects, cookies, and template URLs back to /ppt/...
    # for the browser. Setting root_path here would double-prefix and break
    # StaticFiles mounts (root_path + mount path mismatch).
)
