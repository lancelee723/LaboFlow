...
const normalizeChunkIndex = (index: ChunkIndex): number =>
  typeof index === 'number' ? index : Number(index);

const displayChunkIndex = (index: ChunkIndex): number => normalizeChunkIndex(index) + 1;
...
let down = ref<HTMLAnchorElement | null>(null)
...
const toggleQuestions = (index: ChunkIndex) => {
...
const isExpanded = (index: ChunkIndex) => expandedChunks.value.has(normalizeChunkIndex(index));

// 删除中的状态
const deletingQuestion = ref<{ chunkIndex: number; questionId: string } | null>(null);

// 删除生成的问题
const handleDeleteQuestion = async (item: any, chunkIndex: ChunkIndex, question: GeneratedQuestion) => {
  if (!item || !item.id) {
    MessagePlugin.error(t('common.error'));
    return;
  }
  const normalizedChunkIndex = normalizeChunkIndex(chunkIndex);
  ...
  const confirmDialog = DialogPlugin.confirm({
    ...
    onConfirm: async () => {
      confirmDialog.hide();
      deletingQuestion.value = { chunkIndex: normalizedChunkIndex, questionId: question.id };
      try {
...
// 检查是否正在删除某个问题
const isDeleting = (chunkIndex: ChunkIndex, questionId: string) => {
  return deletingQuestion.value?.chunkIndex === normalizeChunkIndex(chunkIndex) && deletingQuestion.value?.questionId === questionId;
};
...
const isParentExpanded = (index: ChunkIndex) => parentContextExpanded.value.has(normalizeChunkIndex(index));

const isParentContextLoading = (index: ChunkIndex) => parentContextLoading.value.has(normalizeChunkIndex(index));

const toggleParentContext = async (item: any, index: ChunkIndex) => {
  const normalizedIndex = normalizeChunkIndex(index);
  if (parentContextExpanded.value.has(normalizedIndex)) {
    parentContextExpanded.value.delete(normalizedIndex);
  ...
  if (!parentContextCache.value.has(parentId)) {
    parentContextLoading.value.add(normalizedIndex);
    ...
    try {
      ...
      parentContextLoading.value.delete(normalizedIndex);
      parentContextLoading.value = new Set(parentContextLoading.value);
    }
  }

  parentContextExpanded.value.add(normalizedIndex);
  parentContextExpanded.value = new Set(parentContextExpanded.value);
        ...
        <div v-else class="chunk-list">
          <div class="chunk-item" v-for="(chunk, index) in processedChunks" :key="index">
            <div class="chunk-header">
              <span class="chunk-index">{{ $t('knowledgeBase.segment') }} {{ displayChunkIndex(index) }}</span>
            ...
            <div v-if="chunk.hasParent" class="parent-context-section">
              <div class="parent-context-toggle" @click="toggleParentContext(chunk.original, index)">
                <t-icon v-if="!isParentContextLoading(index)"
