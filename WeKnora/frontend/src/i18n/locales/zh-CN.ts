export default {
  ...
  knowledgeBase: {
    ...
    uploadResultSummary: "成功 {success} 个，跳过 {skipped} 个，失败 {failed} 个。",
    uploadSkippedFilesTitle: "已跳过文件",
    uploadFailedFilesTitle: "上传失败文件",
    uploadReasonHiddenFile: "隐藏文件或隐藏目录中的文件已过滤",
    uploadReasonVideoUnsupported: "暂不支持视频上传",
    uploadReasonImageRequiresVlm: "未启用 VLM，图片文件已跳过",
    uploadReasonAudioRequiresAsr: "未启用 ASR，音频文件已跳过",
    uploadReasonNoParser: "当前知识库没有可用解析器或不支持该文件类型",
    uploadFile: "上传文件",
  ...
  imOverview: {
    menuTitle: "已接入的 IM",
    pageTitle: "已接入的 IM",
    subtitle: "当前租户下所有智能体已接入的 IM 渠道，可快速启停或跳转到对应智能体",
    totalCount: "共 {count} 个",
    empty: "当前租户还没有配置 IM 机器人",
